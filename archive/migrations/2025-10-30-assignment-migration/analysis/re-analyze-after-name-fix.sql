-- Re-Analysis After Name Standardization
...
SELECT
  COUNT(*) as total_scis,
  SUM(assignments_count) as total_assignments,
  SUM(initiatives_count) as total_initiatives,
  SUM(unique_assignments_to_migrate) as total_unique_to_migrate,
  SUM(duplicate_assignments) as total_duplicates
FROM sci_summary;

-- Show which SCIs still need migration
SELECT
  canonical_name,
  unique_assignments_to_migrate,
  migration_priority
FROM sci_summary
WHERE unique_assignments_to_migrate > 0
